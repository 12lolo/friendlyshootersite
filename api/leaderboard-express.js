...
app.get('/itch/embed', async (req, res) => {
  try {
        const target = 'https://tjwgames.itch.io/friendlyshooter/embed';
    ...
    // Inject a <base> tag so relative URLs resolve against itch.io
        const baseTag = '<base href="https://tjwgames.itch.io/friendlyshooter/">';
    ...
    res.send(out);
        ...
        res.status(502).send('<html><body style="background:#07120a;color:#fff;padding:24px;">Unable to fetch embed from itch.io. <a href="https://tjwgames.itch.io/friendlyshooter" target="_blank" rel="noopener" style="color:#9ff">Open on itch.io</a></body></html>');
  }
});
...
app.get('/itch/meta', async (req, res) => {
  try {
        const target = 'https://tjwgames.itch.io/friendlyshooter';
